...
Prompt agenta pod wieloma względami kierować się zasadami, które omówiliśmy już w lekcji **S01E01**. Jednocześnie teraz nie skupiamy się wyłącznie na tym, co robi dany agent, ale również jego roli w systemie wieloagentowym oraz zmieniającym się środowisku.
...
Ostatecznie też po sekcji `<tools>` mamy także dynamiczną sekcję **WORKSPACE\_SECTION** do której trafiają treści związane z mechaniką "continous session" oraz "observational memory", które omawialiśmy w poprzednich lekcjach. Zaraz pod nią mamy także ostatnie zdanie, przypominające CTA (Call to Action), którego rola polega na jasnym zasygnalizowaniu zakończenia instrukcji systemowej.
